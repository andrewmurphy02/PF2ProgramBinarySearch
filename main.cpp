...
if (command == "2")
{
    ...
    int position = mYear.iterativeSearch(stoi(targetYear), mYear.gameMachines.size(), stoi(targetYear));

    if (position != -1)
    {
        cout << "Machines manufactured in " << targetYear << ":" << endl;

        while (position < mYear.gameMachines.size() && mYear.gameMachines[position].getMakeYear() == stoi(targetYear))
        {
            mYear.printSingle(position);
            cout << endl;
            position++;
        }
    }
    ...
    {
        cout << "No machines found for the target year." << endl;
    }
}
